<!DOCTYPE html>
<html>
<head>

    <?php require "../header.html" ?>
    <title>CTI - Churchill Training Institute - COURS D’ANGLAIS POUR TOUS a Douala</title>

</head>
<body id="ancre1">
<?php
require '../navbar.html';
?>

<div class="div2">
    <div class="text_div2">COURS D’ANGLAIS POUR TOUS</div>
    <div class="text2_div2"><a class="lien_div2" href="">ACCUEIL</a>><a class="lien_div2" href="">CANDIDATURE</a>> <span class="text_maj">CANDIDATURE A YAOUNDE</span></div>
</div>
<div class="flex-contain">

    <div class="allemand-contain">
        <div class="couleur_fond">
            <img class="float_img img_allem_jeunes"  src="../Resources/img/book.PNG">


            <div class="allemand-top-right">
                <h1 class="pad"> DEVENEZ BILINGUE EN QUELQUES SEMAINES</h1>
                <div class="pad justif_text">
                    Tous nos cours permettent de préparer les candidats aux examens internationaux : TOEFL, IELTS, TOEIC. <br>

                    No cours sont conçus d’après le Cadre Européen Commun de Référence pour les Langues (CECR).<br>
                    Après un test de niveau qui dure 30 minutes, l’apprenant peut être admis à un des niveaux du programme : A1, A2, B1, B2, C1, C2.<br>
                    Nous dispensons des cours d’Anglais général pour tous les niveaux.
                    Que vous soyez élève, étudiant, chercheur d’emplois ou même travailleurs les différents cours d’Anglais que nous dispensons au Churchill Training Institute permettrons d’améliorer ou de perfectionner vos connaissances en anglais, vous donnant ainsi un atout supplémentaire pour votre progrès professionnel ou académique.
                </div>

                <button class="button text_button pad" style="vertical-align:middle"><span><a href="/ile/Views/dates.php"> DATES ET TARIFS </a></span></button>

            </div>

        </div>
        <div class="flex-container3 dessous espace_haut">
            <div class="green_div3">
                <h3><a class="liens_footer" href="">COURS INTENSIFS</a></h3>
                <p>
                    11 Semaines <br>
                    5 Cours par Semaine (3 Unités de cours par jour)<br>
                    Total 165 Unités de cours de 45 Minutes<br>
                    Niveau proposé: A1<br>
                    Max. 24 Adultes
                </p>
                <button class="button text_button pad" style="vertical-align:middle"><span>DATES ET TARIFS &raquo;</span></button>
            </div>
            <div class="green_div4">
                <h2>CONTACT</h2>
                <h3>SECRETARAIT DES COURS</h3>
                <span>Tel / WhatsApp: 673 91 94 90; 695 50 48 82</span> <br>
                <span><a href="">Email: ctidouala@cti-cm.com</a></span>
                <hr>
                <h2>INSCRIPTION</h2>
                <P>
                    Vous êtes intéressé(e) par nos
                    cours de langue ou avez des questions ?
                    Retrouvez nous à notre siège à  <strong>Bonamoussadi, En face du centre des impôts de Douala 5ième.</strong>

                </P>
            </div>
        </div>
        <div class="flex-container3 dessous">
            <div class="green_div3">
                <h3><a class="liens_footer" href="">FORMATION CONTINUE </a></h3>
                <p>
                    12 Semaines<br>
                    5 Cours par Semaine (3 Unités de cours par jour)<br>
                    Total 180 Unités de cours de 45 Minutes<br>
                    Niveau proposé: B2<br>
                    Max. 24 Adultes
                </p>
                <button class="button text_button pad" style="vertical-align:middle"><span>DATES ET TARIFS &raquo;</span></button>
            </div>
        </div>
        <div class="flex-container3 dessous">
            <div class="green_div3">
                <h3><a class="liens_footer" href="">COURS DU SOIR</a></h3>
                <p>
                    8 Semaines<br>
                    5 Cours par Semaine (4 Unités de cours par jour)<br>
                    Total 165 Unités de cours de 45 Minutes<br>
                    Niveau proposé: C1 <br>
                    Max. 24 Adultes
                </p>
                <button class="button text_button pad" style="vertical-align:middle"><span>DATES ET TARIFS &raquo;</span></button>
            </div>
        </div>
        <span class="espace_h">
            <span class="ligne_hori">
                INFORMATIONS COMPLÉMENTAIRES
            </span>
            <span>
                <hr>
            </span>
        </span>
        <div class="flex-container3 dessous2">
            <div class="green_div2">
                <h3><a class="liens_footer3" href="">NOS TYPES DE COURS</a></h3>
                <p>
                    Que cela soit à la maison devant l’ordinateur, dans une
                    salle de classe ou un mélange des deux, grâce à nos
                    formats de cours modernes, vous avez le choix
                </p>
            </div>
            <div class="green_div2">
                <h3><a class="liens_footer3" href="">APERCU NIVEAUX A1-C1</a></h3>
                <p>
                    Nos cours d’allemand sont conçus d’après le Cadre européen commun de référence pour les langues (CECR).
                </p>
            </div>
        </div>
        <span><a class="liens_footer3 marg" href="#ancre1"><strong>HAUT DE PAGE </strong></a></span>
    </div>
</div>


<?php
require '../bande-magnetique.html';
?>
<?php
require '../footer.html';
?>
</body>
<script src="../Resources/js/inscription.js"></script>
</html>